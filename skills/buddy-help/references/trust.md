...
- **Local-first ≠ offline-only.** Notes, notebooks, and session state stay on disk. Network is used for AI providers, web search/fetch, Browser pages, optional MCP, and app updates.
...
- **Browser** loads live websites over the network. Its profiles keep cookies and logins on this machine; Incognito data is cleared when Buddy closes (`browser.md`).
- **Provider and MCP sign-in** can open the system browser for authentication.
...
| “Only model calls need internet” | Incomplete. Browser pages, web search/fetch, MCP, and updates also need network when used. |
...
- **Settings → General → Open files outside the notebook without asking** controls read-only opening of linked external files. It is separate from the permission dock's **Allow always**.
...
Headlines can include **Permission needed: Access external folder**, Read/Edit file, or Run command, with path or command detail. Multiple waits: one card + “+N more pending…”.
...
**Settings → Notifications → Permission notifications**: system notify when Buddy waits on a permission (**Permission required**) or a question (**Question waiting**).
